...
function plotscenarios(
    scen::Array{Float64,2};
    quants=collect(0.05:0.1:0.25),
    savepath::String="",
    save::Bool=false,
    ...
    if prod(isapprox.(med_scen - quantile_scen(scen, 0.0), 0.0, atol=1e-1))
        p1 = plot(
            round.(med_scen, digits=5);
            color="Steel Blue",
            xticks=(
                collect(1:Int(floor(nstag / 4)):nstag),
                [string(i) for i in collect(1:Int(floor(nstag / 4)):nstag)],
            ),
            label="Max and Min",
            kwargs...,
        )
    ...
    p1 = plot(
        med_scen;
        ribbon=(med_scen - quantile_scen(scen, 0.0), quantile_scen(scen, 1.0) - med_scen),
        color="gray",
        xticks=(
            collect(1:Int(floor(nstag / 4)):nstag),
            [string(i) for i in collect(1:Int(floor(nstag / 4)):nstag)],
    ...
    for q in quants
        plot!(
            ...
            med_scen;
            ribbon=(
                med_scen - quantile_scen(scen, q), quantile_scen(scen, 1 - q) - med_scen
            ),
            color="gray",
            label="",
        )
...
function plotresults(results::Dict; nc::Int=3)
    ...
    baseMVA =
        [
            results[:simulations][i][j][:powersystem]["solution"]["baseMVA"] for i in 1:nsim,
            j in 1:nstages
        ]'
    scen_gen = [
        [
            results[:simulations][i][j][:powersystem]["solution"]["gen"]["$gen"]["pg"] for
            i in 1:nsim, j in 1:nstages
        ]' .* baseMVA for gen in 1:ngen
    ]

    plt = [
        plotscenarios(
            scen_gen[gen];
            title="Thermal Generation $gen",
            ylabel="MW",
            xlabel="Stages",
            bottom_margin=10mm,
            right_margin=10mm,
    ...
    if results[:params]["model_constructor_grid_forward"] == PowerModels.ACPPowerModel
        scen_qgen = [
            [
                results[:simulations][i][j][:powersystem]["solution"]["gen"]["$gen"]["qg"]
                for i in 1:nsim, j in 1:results[:params]["stages"]
            ]' .* baseMVA for gen in 1:ngen
        ]
        plt = [
            plotscenarios(
                scen_qgen[gen];
                title="Thermal Reactive Generation $gen",
                ylabel="MW",
                xlabel="Stages",
                bottom_margin=10mm,
                right_margin=10mm,
    ...
    scen_branch = [
        [
            results[:simulations][i][j][:powersystem]["solution"]["branch"]["$brc"]["pf"]
            for i in 1:nsim, j in 1:nstages
        ]' .* baseMVA for brc in 1:nbrc
    ]

    plt = [
        plotscenarios(
            scen_branch[brc];
            title="Branch Flow $brc",
            ylabel="MW",
            xlabel="Stages",
            bottom_margin=10mm,
            right_margin=10mm,
    ...
    if results[:params]["model_constructor_grid_forward"] == PowerModels.ACPPowerModel
        scen_branch_qf = [
            [
                results[:simulations][i][j][:powersystem]["solution"]["branch"]["$brc"]["qf"]
                for i in 1:nsim, j in 1:results[:params]["stages"]
            ]' .* baseMVA for brc in 1:nbrc
        ]
        plt = [
            plotscenarios(
                scen_branch_qf[brc];
                title="Branch Reactive Flow $brc",
                ylabel="MW",
                xlabel="Stages",
                bottom_margin=10mm,
                right_margin=10mm,
    ...
    try
        ...
        scen_va = convert(
            ...
            [
                [
                    results[:simulations][i][j][:powersystem]["solution"]["bus"]["$bus"]["va"]
                    for i in 1:nsim, j in 1:nstages
                ]' for bus in 1:nbus
            ],
        )
    ...
    try
        ...
        scen_pld =
            convert(
                ...
                [
                    [
                        -results[:simulations][i][j][:powersystem]["solution"]["bus"]["$bus"]["lam_kcl_r"]
                        for i in 1:nsim, j in 1:nstages
                    ]' for bus in 1:nbus
                ],
            ) / baseMVA
    ...
    try
        ...
        scen_def = convert(
            ...
            [
                [
                    results[:simulations][i][j][:powersystem]["solution"]["bus"]["$bus"]["deficit"]
                    for i in 1:nsim, j in 1:nstages
                ]' for bus in 1:nbus
            ],
        )
        ...
        plt = [
            plotscenarios(
                scen_def[bus] .* baseMVA;
                title="Deficit bus $bus",
                ylabel="MW",
                xlabel="Stages",
                bottom_margin=10mm,
                right_margin=10mm,
    ...
    scen_turn = convert(
        ...
        [
            [
                results[:simulations][i][j][:reservoirs][:outflow][res] for i in 1:nsim,
                j in 1:nstages
            ]' for res in 1:results[:data][1]["hydro"]["nHyd"]
        ],
    )
    plt = [
        plotscenarios(
            scen_turn[res];
            title="Reservoir Outflow $res",
            ylabel="m³/s",
            xlabel="Stages",
            bottom_margin=10mm,
            right_margin=10mm,
    ...
    scen_spill = convert(
        ...
        [
            [
                results[:simulations][i][j][:reservoirs][:spill][res] for i in 1:nsim,
                j in 1:nstages
            ]' for res in 1:results[:data][1]["hydro"]["nHyd"]
        ],
    )
    ...
    scen_voume = convert(
        ...
        [
            [
                results[:simulations][i][j][:reservoirs][:reservoir][res].out for
                i in 1:nsim, j in 1:nstages
            ]' for res in 1:results[:data][1]["hydro"]["nHyd"]
        ],
    )
    ...
    # Inflows
    scen_inflows = convert(
        Array{Array{Float64,2},1},
        [
            [
                results[:data][1]["hydro"]["Hydrogenerators"][res]["inflow"][
                    cidx(j, results[:data][1]["hydro"]["size_inflow"][1]),
                    results[:simulations][i][j][:noise_term],
                ] for i in 1:nsim, j in 1:nstages
            ]' for res in 1:results[:data][1]["hydro"]["nHyd"]
        ],
    )
    ...
    if mod(nplots, nc) > 0 && floor(Int, nplots / nc) > 0
        l = @layout [
            Plots.grid(floor(Int, nplots / nc), nc)
            Plots.grid(1, mod(nplots, nc))
        ]
        nlines = floor(Int, nplots / nc) + 1
        l.heights =
            Plots.grid(2, 1; heights=[floor(Int, nplots / nc) / nlines; 1 / nlines]).heights
    elseif floor(Int, nplots / nc) > 0
        l = @layout Plots.grid(floor(Int, nplots / nc), nc)
        nlines = floor(Int, nplots / nc)
        l.heights = Plots.grid(nlines, 1; heights=[1 / nlines for n in 1:nlines]).heights
    else
    ...
    return plot(
        plt_total[1:nplots]...;
        layout=l,
        size=(nc * 400, 400 * ceil(Int, nplots / nc)),
        legend=false,
    )
end

"""Common descriptive statistics"""
function descriptivestatistics_results(
    results::Dict; nitem::Int=3, quants::Array{Float64}=[0.25; 0.5; 0.75]
)
    dcp_stats = Dict()
    ...
    if results[:params]["model_constructor_grid"] !=
        PowerModels.AbstractPowerModel{PowerModels.SOCWRForm}
        dcp_stats["va"] = Dict()
    ...
    for i in 1:size(idxhyd[1:min(results[:data][1]["hydro"]["nHyd"], nitem)], 1)
    ...
    for res in 1:min(results[:data][1]["hydro"]["nHyd"], nitem)
        dcp_stats["volume"]["$res"] = quantile_scen(
            scen_voume[res], quants; output_dict=true
        )
    end
...
"""
function plot_grid(
    data::Dict; path=nothing, size_fig=[15cm, 15cm], has_nodelabel=false, nodelabeldist=4.5
)
    ...
    for i in 1:length(data["powersystem"]["gen"])
        ...
        if i in idxhyd
            hydro_nodes[bus_i] +=
                data["powersystem"]["gen"]["$i"]["pmax"] *
                data["powersystem"]["gen"]["$i"]["mbase"]
        else
            thermal_nodes[bus_i] +=
                data["powersystem"]["gen"]["$i"]["pmax"] *
                data["powersystem"]["gen"]["$i"]["mbase"]
    ...
    for i in 1:length(data["powersystem"]["load"])
        ...
        load_nodes[bus_i] +=
            data["powersystem"]["load"]["$i"]["pd"] * data["powersystem"]["baseMVA"]
    end

    # number of nodes
    num_nodes =
        nbus + sum(load_nodes .> 0) + sum(hydro_nodes .> 0) + sum(thermal_nodes .> 0)
    ...
    # node label
    nodelabel = if has_nodelabel
        [
        1:nbus
        fill("", sum(hydro_nodes .> 0))
        fill("", sum(thermal_nodes .> 0))
        fill("", sum(load_nodes .> 0))
    ]
    else
        nothing
    end
...
function plot_hydro_grid(
    data::Dict; path=nothing, size_fig=[12cm, 12cm], has_nodelabel=false, nodelabeldist=8.5
)
    gather_useful_info!(data)
...
function plot_aggregated_results(results::Dict; nc::Int=3)
    ...
    try
        ...
        for bus in idxbus
            scen_pld =
                scen_pld .+ scen_pld_all[bus] .* hcat(fill(load_nodes[:, bus], nsim)...)
        end
        for t in 1:nstages
        ...
        plt = plotscenarios(
            scen_pld;
            title="Load Weighted Average Nodal price ",
            ylabel="\$/MW",
            xlabel="Stages",
            bottom_margin=10mm,
            right_margin=10mm,
    ...
    scen_spill_all =
        ...
        ) ./ (0.0036 * results[:params]["stage_hours"])
    ...
    for res in 1:nHyd
        scen_voume =
            ...
            (
                scen_voume_all[res] .*
                results[:data][1]["hydro"]["Hydrogenerators"][res]["water_energy"]
            ) ./ (0.0036 * results[:params]["stage_hours"])
    end
    ...
    return plot(
        plt_total[1:nplots]...;
        layout=l,
        size=(4 * 400, 500 * ceil(Int, nplots / nc)),
        legend=false,
    )
end
...
function plot_bound(m)
    ...
    val =
        round.(
            [
                m.forward_graph.most_recent_training_results.log[iter].bound for
                iter in 1:niter
            ],
            digits=5,
        )
